...
*                                                                         *
*  File:  SHOP.H                                       Based on CircleMUD *
*  Usage: Structs and Macros for Shops                                    *
*  Programmer(s): Original code by Jeff Fink                              *
...
void create_thiefcatcher(struct char_data *k, struct char_data *t);
...
int is_ok_char(struct char_data * keeper, struct char_data * ch, int shop_nr)
{
  ...
  if ((IS_WIZARD(ch) && NOTRADE_MAGIC_USER(shop_nr)) ||
      (IS_PRIEST(ch) && NOTRADE_CLERIC(shop_nr)) ||
		       ...
		       struct char_data * keeper, int shop_nr)
{
  ...
  int goldamt = 0, buynum, bought = 0, haggle = 0, percent = 101;
  ...
  haggle = buy_price(obj, shop_nr);
  percent = number(10, 101);
  /* 1/2 off sale with successful haggle */
  if (GET_SKILL(ch, SKILL_HAGGLE) > percent)
     haggle /= 2;

  if ((haggle > GET_GOLD(ch)) && !IS_GOD(ch)) {
  ...
  while ((obj) && ((GET_GOLD(ch) >= haggle) || IS_GOD(ch))
			    ...
			    struct char_data * keeper, int shop_nr, int msg)
{
  ...
  if (GET_OBJ_VAL(obj, 9) > material_list[GET_OBJ_VAL(obj,7)].sturdiness) {
    sprintf(buf, "%s  I wouldn't want that piece of junk.  Try and sell it to the smithy.", GET_NAME(ch));
    do_tell(keeper, buf, cmd_tell, 0);
    return (0);
  }
			...
			struct char_data * keeper, int shop_nr)
{
  ...
  if (GET_GOLD(keeper) < MIN_OUTSIDE_BANK) {
    goldamt = MIN((long)MAX_OUTSIDE_BANK - GET_GOLD(keeper), (long)SHOP_BANK(shop_nr));
...
SPECIAL(shop_keeper)
{
  ...
  if (CMD_IS("steal")) {
    /* tricky code for when PC's try to steal shit from keeper */
    sprintf(argm, "Help! Help! %s is trying to steal from me!", GET_NAME(ch));
    do_gen_comm(keeper, argm, 0, SCMD_SHOUT);
    create_thiefcatcher(keeper, ch);
...
void boot_the_shops(FILE * shop_f, char *filename, int rec_count)
{
  ...
  while (!done) {
    ...
    if (*buf == '#') {          /* New shop */
      sscanf(buf, "#%ld\n", &temp);
      sprintf(buf2, "shop #%ld in shop file %s", temp, filename);
...
void list_all_shops(struct char_data * ch)
{
  ...
  for (shop_nr = 0; shop_nr < top_shop; shop_nr++) {
    ...
    sprintf(buf2, "%3d   %-7ld   %-7ld    ", shop_nr + 1, SHOP_NUM(shop_nr),
	    SHOP_ROOM(shop_nr, 0));
    if (SHOP_KEEPER(shop_nr) < 0)
      strcpy(buf1, "<NONE>");
    else
      sprintf(buf1, "%-7ld", mob_index[SHOP_KEEPER(shop_nr)].vnumber);
...
void list_detailed_shop(struct char_data * ch, int shop_nr)
{
  ...
  for (index = 0; SHOP_ROOM(shop_nr, index) != NOWHERE; index++) {
    ...
    if ((temp = real_room(SHOP_ROOM(shop_nr, index))) != NOWHERE)
      sprintf(buf1, "%s (#%-7ld)", world[temp].name, world[temp].number);
    else
      sprintf(buf1, "<UNKNOWN> (#%-7ld)", SHOP_ROOM(shop_nr, index));
  ...
  if (SHOP_KEEPER(shop_nr) >= 0) {
    sprintf(END_OF(buf), "%s (#%-7ld), Special Function: %s\n\r",
	    GET_NAME(&mob_proto[SHOP_KEEPER(shop_nr)]),
	mob_index[SHOP_KEEPER(shop_nr)].vnumber, YESNO(SHOP_FUNC(shop_nr)));
    if ((k = get_char_num(SHOP_KEEPER(shop_nr)))) {
      send_to_char(buf, ch);
      sprintf(buf, "Coins:      [%-7ld], Bank: [%d] (Total: %-7ld)\n\r",
  ...
  for (index = 0; SHOP_PRODUCT(shop_nr, index) != NOTHING; index++) {
    ...
    sprintf(buf1, "%s (#%ld)", obj->short_description,
	    obj_index[SHOP_PRODUCT(shop_nr, index)].vnumber);
    handle_detailed_list(buf, buf1, ch);
  }
  if (!index)
