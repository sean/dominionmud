...
*                                                                         *
*  File:  ACT_SOC.C                                    Based on CircleMUD *
*  Usage: Functions used to handle socials                                *
*  Programmer(s): Original work by Jeremy Elson (Ras)                     *
*                 All modifications by Sean Mountcastle (Glasgian)        *
*                 For The Dominion.                                       *
\*********************************************************************** */

#define __ACT_SOCL_C__
...
ACMD(do_tell);
void mprog_wordlist_check(char *arg, struct char_data * mob, struct 
    char_data *actor, struct obj_data * obj, void *vo, int type);       
...
ACMD(do_insult)
{
  ...
  if (*arg) {
	...
	case 0:
	  if (GET_SEX(ch) == SEX_MALE) {
	    ...
	    if (GET_SEX(victim) == SEX_MALE)
	      ...
	      sprintf(buf, "%s You'd lose a beauty contest against a troll.", 
			GET_NAME(victim));
	      do_tell(ch, buf, 0, 0);
	    }
	  }
	...
	case 1:
	  ...
	  break;
	default:
	  sprintf(buf, "%s Get lost, loser!", GET_NAME(victim));
	  do_tell(ch, buf, 0, 0);
	  break;
	}                       /* end switch */
