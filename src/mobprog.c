		     ...
		     struct obj_data *obj, void *vo, struct char_data *rndm)
{
  ...
  if (*point == '\0')
    {
      bug ("Mob: %ld null ifchck", (long)mob_index[mob->nr].vnumber);
      return -1;
    }
  /* skip leading spaces */
  while (*point == ' ')
  ...
  while (*point != '(')
    if (*point == '\0')
      {
	bug ("Mob: %ld ifchck syntax error", mob_index[mob->nr].vnumber);
	return -1;
      }
    else
      if (*point == ' ')
  ...
  *argpt = '\0';
  ...
  if (!str_cmp(buf, "hunting")) {
      switch (arg[1])  /* arg should be "$*" so just get the letter */
	{
	case 'i': return (HUNTING(mob) == mob ? 1 : 0);
	case 'n': if (actor)
		    return (HUNTING(mob) == actor ? 1 : 0);
		  else 
		    return -1;
	case 't': if (vict)
		    return (HUNTING(mob) == vict ? 1 : 0);
		  else 
		    return -1;
	case 'r': if (rndm)
		    return (HUNTING(mob) == rndm ? 1 : 0);
		  else 
		    return -1;
	default:
	  bug ("Mob: %ld bad argument to 'hunting'",
		mob_index[mob->nr].vnumber);
	  return -1;
	}
    }
	...
	case 'r': if (rndm)
		  {
		    ...
		    return -1;
	default:
	  bug ("Mob: %ld bad argument to 'class'", mob_index[mob->nr].vnumber);
	  return -1;
	}
    }
  if (!str_cmp(buf, "time"))
    {
      lhsvl = (time_info.hours);
      rhsvl = atoi(val);
      if (lhsvl > 24 || lhsvl < 1)
	return -1;
      else
	...
	case 'r': if (rndm)
		  {
		    ...
		    return -1;
	default:
	  bug ("Mob: %ld bad argument to 'goldamt'", mob_index[mob->nr].vnumber);
	  return -1;
	}
    }
	...
	case 'p': if (v_obj)
		  {
		    ...
		    return -1;
	default:
	  bug ("Mob: %ld bad argument to 'objtype'", mob_index[mob->nr].vnumber);
	  return -1;
	}
    }
	...
	case 'p': if (v_obj)
		  {
		    ...
		    return -1;
	default:
	  bug ("Mob: %ld bad argument to 'objval0'", mob_index[mob->nr].vnumber);
	  return -1;
	}
    }
	...
	case 'p': if (v_obj)
		  {
		    ...
		    return -1;
	default:
	  bug ("Mob: %ld bad argument to 'objval1'", mob_index[mob->nr].vnumber);
	  return -1;
	}
    }
	...
	case 'p': if (v_obj)
		  {
		    ...
		    return -1;
	default:
	  bug ("Mob: %ld bad argument to 'objval2'", mob_index[mob->nr].vnumber);
	  return -1;
	}
    }
	...
	case 'p': if (v_obj) 
		  {
		    ...
		    return -1;
	default:
	  bug ("Mob: %ld bad argument to 'objval3'", mob_index[mob->nr].vnumber);
	  return -1;
	}
    }
  ...
  if (!str_cmp(buf, "number"))
	...
	case 'n': if (actor)
		  {
		    if IS_NPC(actor)
		    {
		      lhsvl = mob_index[actor->nr].vnumber;
		      rhsvl = atoi(val);
		      return mprog_veval(lhsvl, opr, rhsvl);
		    }
		  }
	...
	case 't': if (vict)
		  {
		    if IS_NPC(actor)
		    {
		      lhsvl = mob_index[vict->nr].vnumber;
		      rhsvl = atoi(val);
		      return mprog_veval(lhsvl, opr, rhsvl);
		    }
		  }
	...
	case 'r': if (rndm)
		  {
		    if IS_NPC(actor)
		    {
		      lhsvl = mob_index[rndm->nr].vnumber;
		      rhsvl = atoi(val);
		      return mprog_veval(lhsvl, opr, rhsvl);
		    }
		  }
	...
	case 'o': if (obj)
		  {
		    lhsvl = obj_index[obj->item_number].vnumber;
		    rhsvl = atoi(val);
		    return mprog_veval(lhsvl, opr, rhsvl);
		  }
		  else
	...
	case 'p': if (v_obj)
		  {
		    lhsvl = obj_index[v_obj->item_number].vnumber;
		    rhsvl = atoi(val);
		    return mprog_veval(lhsvl, opr, rhsvl);
		  }
		  else
		    return -1;
	default:
	  bug ("Mob: %ld bad argument to 'number'", mob_index[mob->nr].vnumber);
	  return -1;
	}
    }
  ...
  if (!str_cmp(buf, "name"))
	...
	case 'p': if (v_obj)
		    ...
		    return -1;
	default:
	  bug ("Mob: %ld bad argument to 'name'", mob_index[mob->nr].vnumber);
	  return -1;
	}
    }
   ...
   */
  bug ("Mob: %ld unknown ifchck", mob_index[mob->nr].vnumber);
  return -1;
 ...
 *null = '\0';
 ...
 while(loopdone == FALSE) /*scan over any existing or statements */
 {
     ...
     if (*cmnd == '\0')
     {
	 bug ("Mob: %ld no commands after IF/OR", mob_index[mob->nr].vnumber);
	 return null;
     }
 ...
 if (flag)
   for (; ;) /*ifcheck was true, do commands but ignore else to endif*/ 
   {
	 ...
	 cmnd++;
       if (*cmnd == '\0')
       {
	   bug ("Mob: %ld missing else or endif", mob_index[mob->nr].vnumber);
	   return null;
       }
       morebuf = one_argument(cmnd, buf);
   }
 else /*false ifcheck, find else and do existing commands or quit at endif*/
   {
     ...
     if (*cmnd == '\0')
       {
	 bug ("Mob: %ld missing endif", mob_index[mob->nr].vnumber);
	 return null;
       }
     morebuf = one_argument(cmnd, buf);
...
/* static char *sir_madam     [] = { "sir", "sir", "madam" }; */
 struct char_data   *vict             = (struct char_data *) vo;
 struct obj_data    *v_obj            = (struct obj_data  *) vo;
 ...
 *t = '\0';
 switch (ch) {
     ...
     default:
	 bug("Mob: %ld bad $var", mob_index[mob->nr].vnumber);
	 break;
       }
