...
extern struct race_data * races;                        /*. race.c      .*/
...
ACMD(do_olc_master)
{
  ...
  /*. No screwing around .*/
  if (IS_NPC(ch)) {
    send_to_char( "The OLC code is very fragile, no screwing around!\r\n", ch );
    return;
  }
  ...
  if (!*buf1) {
	...
	break;
      ...
      case SCMD_OLC_MEDIT:
      case SCMD_OLC_SEDIT:
	sprintf(buf, "Specify %s %s VNUM to edit.\r\n",
                XANA(olc_scmd_info[subcmd].text),
	...
	break;
      case SCMD_OLC_HEDIT:
        send_to_char( "You must specify a help keyword or new.\r\n",
                      ch );
        break;
      default:
        // nothing is alright
  ...
  for (d = descriptor_list; d; d = d->next)
    if (STATE(d) == CON_EDITTING && SUBSTATE(d) == olc_scmd_info[subcmd].con_type)
  ...
  /*. Ensure that there is no existing junk .*/
  ...
  if (save) {
	...
	break;
      default:
        mudlog("ERROR: Unknown save command!", CMP, LVL_BUILDER, TRUE);
        break;
    }
    cleanup_olc( d, CLEANUP_ALL );
    return;
  ...
  switch (subcmd) {
	...
	redit_setup_new(d);
      STATE(d) = CON_EDITTING;
      SUBSTATE(d) = OLC_ROOM_EDIT;
      break;
    case SCMD_OLC_ZEDIT:
      real_num = real_room(number);
      if (real_num < 0) {
	...
	if (gptr && (gptr->number == number)) {
      ...
      STATE(d) = CON_EDITTING;
      SUBSTATE(d) = OLC_GUILD_EDIT;
      break;
    case SCMD_OLC_HEDIT:
      if (!strcmp(hedit_entry, "new"))
        hedit_setup_new(ch->desc);
      else {
        int i = 0;
        struct help_index_element * h = NULL;
        extern int top_of_helpt;
        extern struct help_index_element *help_table;
        extern int top_of_wizhelpt;
        extern struct help_index_element *wizhelp_table;
        // locate the requested help entry, first check in the help
        // table and then in the wizhelp_table
        for ( i = 0; i < top_of_helpt &&
                strn_cmp(hedit_entry,
                         help_table[i].keyword,
                         strlen(hedit_entry)); i++ )
          ;
        if ( !strn_cmp(hedit_entry, help_table[i].keyword,
                       strlen(hedit_entry)) )
          h = help_table + i;

        for ( i = 0; h == NULL && i < top_of_wizhelpt; i++ )
          if ( !strn_cmp(hedit_entry, wizhelp_table[i].keyword,
                         strlen(hedit_entry)) )
            h = wizhelp_table + i;
	  ...
	  send_to_char("Invalid help keyword!\r\n", d->character);
          cleanup_olc( d, CLEANUP_ALL );
	  return;
        }
      }
      STATE(d) = CON_EDITTING;
      SUBSTATE(d) = OLC_HELP_EDIT;
      break;
    default:
      break;
  }
...
void olc_saveinfo(struct char_data *ch)
{
   ...
   if (olc_save_list)
     ...
     send_to_char("The world database is up to date, nothing to save.\r\n", ch);
...
void zedit_parse(struct descriptor_data * d, char *arg)
{
	...
	*/
     ...
     case ZEDIT_ZONE_RESET:
       ...
       pos = atoi(arg);
       if (!isdigit((int)*arg) || (pos <  0) || (pos > 2))
...
void medit_save_internally(struct descriptor_data *d)
{
   ...
   if (rmob_num != -1) {
     ...
     CREATE(new_proto, struct char_data, top_of_mobt + 3);
     CREATE(new_index, struct index_data, top_of_mobt + 3);
     ...
     for (dsc = descriptor_list; dsc; dsc = dsc->next) {
       if (STATE(dsc) == CON_EDITTING && SUBSTATE(dsc) == OLC_SHOP_EDIT)
	...
	*pcDst = '\0';
	...
	if (pstMProg->type == IN_FILE_PROG)
	  fprintf(mob_file, ">in_file_prog %s~\n", pstMProg->arglist);
          ...
          sprintf( buf, "OLC: Problem saving mob prog for %s (%ld)",
                   GET_SDESC(mob), i );
          mudlog( buf, CMP, LVL_BUILDER, TRUE );
        }
...
void medit_disp_menu(struct descriptor_data * d)
{
  ...
  sprintf(buf,
	...
	grn, nrm, cyn, buf1,
	grn, nrm, cyn, buf2,
	grn, nrm, yel, (!RACE_UNDEF(mob) ? races[ GET_RACE(mob) ].name : "(null)"),
        grn, nrm, yel, (GET_HEIGHT(mob) / 12), (GET_HEIGHT(mob) % 12),
        grn, nrm, yel, GET_WEIGHT(mob),
	grn, nrm, yel, GET_STR(mob), GET_INT(mob), GET_WIS(mob), 
                       GET_DEX(mob), GET_CON(mob), GET_CHA(mob), GET_WILL(mob),
	grn, nrm
  );
  send_to_char(buf, d->character);
...
void gedit_setup_new(struct descriptor_data *d)
{
   ...
   if ((OLC_GUILD(d) = enqueue_guild()) != NULL) {
     ...
     OLC_GUILD(d)->number = num_of_guilds++;
     OLC_GUILD(d)->restrictions = 0;
  ...
  switch (OLC_MODE(d)) {
    case HEDIT_CONFIRM_SAVE:
      switch (*arg) {
        ...
        case 'Y':
          /* NOT IMPLEMENTED YET */
          send_to_char("Help saved to disk and memory.\r\n", d->character);
          cleanup_olc(d, CLEANUP_STRUCTS);
          break;
        case 'n':
        case 'N':
        ...
        default:
          send_to_char("Invalid choice!\r\n"
                       "Do you wish to save this guild? : ", d->character);
          break;
      }
      return;
    case HEDIT_MAIN_MENU:
      switch (*arg) {
        ...
        case 'Q':
          if (OLC_VAL(d)) {
            /*. Something has been modified .*/
            send_to_char("Do you wish to save this help entry? : ", d->character);
            OLC_MODE(d) = HEDIT_CONFIRM_SAVE;
          } else
            cleanup_olc(d, CLEANUP_STRUCTS);
          return;
        default:
          send_to_char( "Sorry this feature isn't fully implemented yet.\r\n",
                        d->character );
          break;
      }
      break;
...
{
  assert( d != NULL );
  assert( OLC_HELP(d) != NULL );
  
  get_char_cols(d->character);

  sprintf(buf,
          "[H[J"
          "%s1%s) Help Keyword(s) : [%s%s%s]\r\n"
          "%s2%s) Entry :\r\n%s%s%s\r\n"
          "%sP%s) Purge this Help entry\r\n"
          "%sQ%s) Quit\r\n"
          "Enter choice : ",
          grn, nrm, cyn, OLC_HELP(d)->keyword, nrm,
          grn, nrm, cyn, OLC_HELP(d)->entry, nrm,
          grn, nrm,
          grn, nrm );
  send_to_char(buf, d->character);
