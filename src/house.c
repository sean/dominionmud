...
int can_add_guest(struct char_data *ch, long id);
...
int House_get_filename(long vnum, char *filename)
{
  if (vnum < 0)
    return 0;
...
void House_delete_file(long vnum)
{
...
void House_listrent(struct char_data * ch, long vnum)
{
  ...
  if (!House_get_filename(vnum, fname))
    return;
  if (!(fl = fopen(fname, "rb"))) {
    sprintf(buf, "No objects on file for house #%ld.\r\n", vnum);
    send_to_char(buf, ch);
    return;
  }
  *buf = '\0';
...
void hcontrol_list_houses(struct char_data * ch)
{
  ...
  for (i = 0; i < num_of_houses; i++) {
    ...
    sprintf(buf, "%s%7ld %7ld  %-10s    %2d    %-12s %s\r\n", buf,
...
ACMD(do_house)
{
  ...
  if (!IS_SET(ROOM_FLAGS(ch->in_room), ROOM_HOUSE))
    ...
    if (can_add_guest(ch, id)) {
       j = house_control[i].num_of_guests++;
       house_control[i].guests[j] = id;
       House_save_control();
       send_to_char("Guest added.\r\n", ch);
    } else
       send_to_char("Guest is too far away in level from you. Not added.\r\n", ch);
  }
}

/* Returns 1 if PC can add player as guest - based on level diff */
int can_add_guest(struct char_data *ch, long id)
{
   int i;
   struct char_file_u plr_rec;

   if (load_char(get_name_by_id(id), &plr_rec) > -1) {
      i = plr_rec.level - GET_LEVEL(ch);
      if (i >= -5 && i <= 5)
	 return 1;
   }
   return 0;
