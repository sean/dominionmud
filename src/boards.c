...
*                                                                         *
*  File:  BOARDS.C                                     Based on CircleMUD *
...
struct board_info_type board_info[NUM_OF_BOARDS] = {
  ...
  {3098, LVL_IMMORT, LVL_IMMORT, LVL_SUP,  "etc/board.immort", 0},
  {3097, 0, LVL_FREEZE, LVL_SUP, "etc/board.freeze", 0},
...
/*    for (i = num_of_msgs[board_type] - 1; i >= 0; i--) { */
	...
	sprintf(buf + strlen(buf), "%s%-2d%s : %s%s%s\r\n",
		CCBBL(ch, C_SPR), i + 1, CCNRM(ch, C_SPR),
		CCBGR(ch, C_SPR), MSG_HEADING(board_type, i), CCNRM(ch, C_SPR));
      else {
...
int Board_display_msg(int board_type, struct char_data * ch, char *arg)
{
  ...
  sprintf(buffer, "Message %s%d%s : %s%s%s\r\n\r\n%s\r\n", 
	  CCBBL(ch, C_SPR), msg, CCNRM(ch, C_SPR),
	  CCCYN(ch, C_SPR), MSG_HEADING(board_type, ind), CCNRM(ch, C_SPR),
