...
*                                                                         *
*  File:  GRAPH.C                                      Based on CircleMUD *
*  Usage: Various graph algorithms used for tracking                      *
...
void bfs_enqueue(ln_int room, char dir)
{
...
int find_first_step(ln_int src, ln_int target)
{
  int curr_dir;
  ln_int curr_room;
...
void hunt_victim(struct char_data * ch)
{
  ...
  if ((dir < 0) && (ch->in_room != HUNTING(ch)->in_room)) {
    if (IS_HUMANOID(ch))  {
      sprintf(buf2, "Damn!  Lost %s!", HMHR(HUNTING(ch)));
      do_say(ch, buf2, 0, 0);
      sprintf(buf2, "I'll get you eventually %s!", GET_NAME(HUNTING(ch)));
    ...
    if (CAN_SPEAK(ch))
      act("$N says, 'I'm gonna kill you, $n!'", FALSE, HUNTING(ch), NULL, 
		ch, TO_NOTVICT);
    /* now hit the person */
